...
namespace printerApp
{
    public class clsDocs
    {
        public static void Receipt(string saleId, string printerName, int paperSize = 80)
        {
            ...
            //ticket1.SetLogo()
            ticket1.TextoCentrado58(dtCompany.Rows[0].Field<string>("name").Trim().ToUpper());//nombre de la compañia
            ticket1.TextoCentrado58(dtCompany.Rows[0].Field<string>("address").Trim().ToUpper());// direccion
            ticket1.TextoCentrado58("RFC: " + dtCompany.Rows[0].Field<string>("taxpayer_id").Trim().ToUpper());// RFC
            ticket1.TextoCentrado58("TELEFONO: " + dtCompany.Rows[0].Field<string>("phone").Trim());//telefono

            ticket1.TextoIzquierda("");
            ticket1.TextoIzquierda("FOLIO #" + saleId);
            ticket1.TextoIzquierda("FECHA: " + fecha.ToString("dd/MM/yy hh:mm tt"));
            ticket1.TextoIzquierda("ATENDIO: " + dtVenta.Rows[0].Field<string>("user").Trim().ToUpper());
            ...
            for (int i = 0; i <= dtDetalle.Rows.Count -1; i++)
            {
                ...
                string fulCad = dtDetalle.Rows[i].Field<string>("product").ToString() + "Cant:" + decimal.Truncate(cant) + "Subt:" + decimal.Round((cant * precio), 2);
                if(fulCad.Length <= 32)
                {
                    ticket1.TextoIzquierda(fulCad);
                } else
                {
        ...
        public static void CreateProtocol()
        {
            ...
            if(key == null)
            {
                ...
                subkeyCommand.SetValue(null, "\"C:\\apps\\tickets\\printerApp.exe\" \"%1\"");

                MessageBox.Show("PROTOCOLO REGISTRADO EN EL SISTEMA OPERATIVO ;)", "PinterApp", MessageBoxButtons.OK, MessageBoxIcon.Information);
